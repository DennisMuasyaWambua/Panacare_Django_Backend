...
class PatientJournal(models.Model):
    """
    Patient journal model for personal health journaling
    """
    id = models.CharField(max_length=20, primary_key=True, editable=False)
    patient = models.ForeignKey('users.Patient', on_delete=models.CASCADE, related_name='journals')
    title = models.CharField(max_length=255)
    content = models.TextField()
    ...
    def save(self, *args, **kwargs):
        if not self.id:
            import uuid
            self.id = f"jn_{str(uuid.uuid4()).replace('-', '')[:9]}"
